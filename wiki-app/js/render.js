...
const R = {
      ...
      <div class="card">
        ...
        ${b.purity_levels?.length ? `<div class="mb-12">
          ...
          ${this.arr(b.purity_levels).map(pl => `
            <div class="flex justify-between mb-8 card-sm" style="border:1px solid var(--border);border-radius:6px">
              <strong class="text-sm">${pl.level}</strong>
              <div>${this.arr(pl.abilities).map(a => `<span class="badge badge-accent">${a}</span>`).join(' ')}</div>
            </div>`).join('')}
        </div>` : ''}
  ...
  analysis(novel) {
    ...
    if (!an) return `<div class="fade-in">
      ...
      ${this.emptyState('No Analysis Data', 'Run the Analysis Mode in Qwen and import the result.', 'bi-cpu')}
    </div>`;
    ...
    return `<div class="fade-in">
      <div class="page-title"><i class="bi bi-cpu"></i>AI Analysis</div>
      <div class="page-subtitle">Deep-dive analysis generated by Qwen</div>
